...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "LINCNetwork") type = LINCNetwork;
    if(strType == "LINCNetworkVariable") type = LINCNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == LINCNetwork) s = "LINCNetwork";
    if(type == LINCNetworkVariable) s = "LINCNetworkVariable";
