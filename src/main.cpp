...
#if defined(NDEBUG)
# error "LINC Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= 6850000) { // max supply
        nSubsidyBase = 0;
        return nSubsidyBase * COIN;
    }
    ...
    for (int i = consensusParams.nSubsidyHalvingInterval; i <= nPrevHeight && nSubsidy >= 5 * COIN; i += consensusParams.nSubsidyHalvingInterval) {
        nSubsidy -= nSubsidy/20; // 5% decrease
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight >= Params().GetConsensus().nMasternodePaymentsStartBlock) {
        ...
        for (int i = Params().GetConsensus().nMasternodePaymentsStartBlock+Params().GetConsensus().nMasternodePaymentsIncreasePeriod, j = 30; i < nHeight && j <= mnMax;  i += Params().GetConsensus().nMasternodePaymentsIncreasePeriod, j+=5) {
            ret += blockValue / 20;
        }
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    CAmount nBlockSubsidy = GetBlockSubsidy(pindex->pprev->nBits, pindex->pprev->nHeight, chainparams.GetConsensus());
    CAmount blockReward = nFees + nBlockSubsidy;
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(LINC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(LINC): couldn't find masternode or superblock payments"),
...
bool IsDpmTransactionValid(const CTransaction& txNew) {
    ...
    BOOST_FOREACH(CTxOut txout, txNew.vout) {
        if (dpmPayee == txout.scriptPubKey && dpmPayment == txout.nValue) {
            LogPrintf("IsDpmTransactionValid -- Found required payment: %s\n", txNew.ToString().c_str());
            return true;
        }
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // LINC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(LINC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(LINC): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        LINC Related Inventory Messages
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - Params().MaxTipAge()) { // NOTE: was "close to today" and 24h in Bitcoin
