...
const char * const BITCOIN_CONF_FILENAME = "linc.conf";
const char * const BITCOIN_PID_FILENAME = "lincd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "linc" is a composite category enabling all LINC-related debug output
            if(ptrCategory->count(string("linc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LINC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LINC
    // Mac: ~/Library/Application Support/LINC
    // Unix: ~/.linc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LINC";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LINC";
#else
    // Unix
    return pathRet / ".linc";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty linc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override linc.conf
